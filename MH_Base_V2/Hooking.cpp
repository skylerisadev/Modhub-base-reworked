...
|  $$  | $$__  $$ /$$__  $$ /$$__  $$| $$$    /$$$   /$$/
 \  $$ | $$  \ $$| $$  \ $$| $$  \ $$| $$$$  /$$$$  /$$/ 
  ...
  /$$/ | $$  \ $$| $$  | $$| $$  | $$| $$\  $ | $$ \  $$ 
 /$$/  | $$$$$$$/|  $$$$$$/|  $$$$$$/| $$ \/  | $$  \  $$
|__/   |_______/  \______/  \______/ |__/     |__/   \__/
	...
	*/
	...
	{
		if (std::strcmp(label, "HUD_JOINING") == 0) return "Loading GTA Online with Atomic Base";
		if (std::strcmp(label, "HUD_TRANSP") == 0) return "Loading with Atomic Base";
		if (std::strcmp(label, "PM_ENTER_MP") == 0) return "Play GTA Online with Atomic Base";
		if (std::strcmp(label, "NT_INV_MP_SAVE") == 0) return "Joining ~g~friend~w~?";
		if (std::strcmp(label, "PM_SCR_STA") == 0) return "Atomic Base";
		if (std::strcmp(label, "RESPAWN_W_MP") == 0) return "Imagine dying...";
		if (std::strcmp(label, "PM_INF_PGOB") == 0) return "Atomic Base";
		if (std::strcmp(label, "PM_INF_PGOB0") == 0) return "Atomic Base";
		if (std::strcmp(label, "PM_PAUSE_HDR") == 0) return "Atomic Base";
		if (std::strcmp(label, "PM_INF_STOREB") == 0) return "~r~Just Dont WASTE UR MONEY.";
		if (std::strcmp(label, "PM_GO") == 0) return "~y~Go Online Atomic Base";
